...
import { Plus, Bell, Send, Users, BookOpen, Briefcase, GraduationCap, Pencil, Trash2, X, AlertTriangle, MessageCircle } from 'lucide-react';
...
const emptyWhatsAppForm = { message: '', targetMode: 'all', targetAll: true, targetCourses: [], studentIds: [] };
...
export default function ManageNotifications() {
  ...
  const openWhatsApp = () => {
    ...
    if (students.length === 0) {
      ...
      api.get('/students?limit=200')
        ...
        .catch(() => toast.error('Failed to load students for selection'))
  ...
  const toggleWhatsAppCourse = (course) => {
    setWhatsAppForm(p => {
      const current = p.targetCourses || [];
      const targetCourses = current.includes(course)
        ? current.filter(c => c !== course)
  ...
  const toggleWhatsAppStudent = (studentId) => {
    setWhatsAppForm(p => {
      const current = p.studentIds || [];
      const studentIds = current.includes(studentId)
        ? current.filter(id => id !== studentId)
        : [...current, studentId];
      return { ...p, studentIds, targetMode: 'students', targetAll: false, targetCourses: [] };
    });
  };

  const sendWhatsAppOnly = async (e) => {
    e.preventDefault();
    if (!whatsAppForm.message.trim()) return toast.error('Please enter a WhatsApp message');
    setWhatsAppSending(true);
    try {
      const payload = { message: whatsAppForm.message.trim() };
      if (whatsAppForm.targetMode === 'courses') {
        payload.targetAll = false;
        payload.targetCourses = whatsAppForm.targetCourses || [];
        payload.targetCourse = payload.targetCourses[0];
      } else if (whatsAppForm.targetMode === 'students') {
        if (!whatsAppForm.studentIds.length) {
          toast.error('Select at least one student');
          setWhatsAppSending(false);
          return;
        }
        payload.targetAll = false;
        payload.studentIds = whatsAppForm.studentIds;
      } else {
        payload.targetAll = true;
      }
      const res = await api.post('/notifications/whatsapp/bulk', payload);
      const delivery = res.data.delivery;
      toast.success(`WhatsApp sent: ${delivery?.whatsappSent || 0}. Skipped: ${delivery?.whatsappSkipped || 0}.`);
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex items-center justify-between scroll-reveal">
        ...
        <div className="flex items-center gap-2">
          <button onClick={openWhatsApp} className="btn-secondary flex items-center gap-2">
      ...
      {showWhatsAppModal && (
        <div className="fixed inset-0 modal-overlay flex items-center justify-center z-50 p-4">
          <div className="modal-card w-full max-w-lg">
            <div className="flex items-center justify-between p-6 border-b" style={{ borderColor: 'var(--border-glow)' }}>
              <h3 className="font-bold flex items-center gap-2" style={{ fontFamily: 'Orbitron, sans-serif', color: 'var(--neon-green)', fontSize: '0.9rem', letterSpacing: '0.08em' }}>
                <MessageCircle size={18} /> WHATSAPP MESSAGE
              </h3>
              <button onClick={() => setShowWhatsAppModal(false)} style={{ color: 'var(--text-muted)' }}><X size={18} /></button>
            </div>
            <form onSubmit={sendWhatsAppOnly} className="p-6 space-y-4">
              <div>
                <label className="label">Message *</label>
                <textarea
                  className="input"
              ...
              <div>
                ...
                <div className="space-y-2">
                  <label className="flex items-center gap-2 cursor-pointer" style={{ color: 'var(--text-secondary)', fontFamily: 'Rajdhani, sans-serif' }}>
                    <input type="radio" checked={whatsAppForm.targetMode === 'all'} onChange={() => setWhatsAppForm(p => ({ ...p, targetMode: 'all', targetAll: true, targetCourses: [], studentIds: [] }))} style={{ accentColor: 'var(--neon-green)' }} />
                    <Users size={14} /> All Students
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer" style={{ color: 'var(--text-secondary)', fontFamily: 'Rajdhani, sans-serif' }}>
                    <input type="radio" checked={whatsAppForm.targetMode === 'courses'} onChange={() => setWhatsAppForm(p => ({ ...p, targetMode: 'courses', targetAll: false, targetCourses: ['B.Tech'], studentIds: [] }))} style={{ accentColor: 'var(--neon-green)' }} />
                    <BookOpen size={14} /> Specific Course(s)
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer" style={{ color: 'var(--text-secondary)', fontFamily: 'Rajdhani, sans-serif' }}>
                    <input type="radio" checked={whatsAppForm.targetMode === 'students'} onChange={() => setWhatsAppForm(p => ({ ...p, targetMode: 'students', targetAll: false, targetCourses: [], studentIds: p.studentIds || [] }))} style={{ accentColor: 'var(--neon-green)' }} />
                    <Users size={14} /> Selected Students
                  </label>
                  {whatsAppForm.targetMode === 'courses' && (
                    <div className="ml-5 grid grid-cols-2 gap-2 pt-1">
                      {COURSES.map(course => (
                        <label key={course} className="flex items-center gap-2 cursor-pointer p-2 rounded-lg"
                          style={{ color: 'var(--text-secondary)', border: '1px solid var(--border-glow)', background: (whatsAppForm.targetCourses || []).includes(course) ? 'rgba(0,255,136,0.08)' : 'transparent' }}>
                          <input type="checkbox" checked={(whatsAppForm.targetCourses || []).includes(course)} onChange={() => toggleWhatsAppCourse(course)} style={{ accentColor: 'var(--neon-green)' }} />
                          <span className="text-sm">{course}</span>
                        </label>
                      ))}
                    </div>
                  )}
                  {whatsAppForm.targetMode === 'students' && (
                    <div className="ml-5 max-h-48 overflow-y-auto space-y-2 pr-1">
                      {studentsLoading ? (
                        <div className="text-sm" style={{ color: 'var(--text-muted)' }}>Loading students...</div>
                      ) : students.length === 0 ? (
                        <div className="text-sm" style={{ color: 'var(--text-muted)' }}>No students found</div>
                      ) : students.map(student => (
                        <label key={student._id} className="flex items-center gap-2 cursor-pointer p-2 rounded-lg"
                          style={{ color: 'var(--text-secondary)', border: '1px solid var(--border-glow)', background: (whatsAppForm.studentIds || []).includes(student._id) ? 'rgba(0,255,136,0.08)' : 'transparent' }}>
                          <input type="checkbox" checked={(whatsAppForm.studentIds || []).includes(student._id)} onChange={() => toggleWhatsAppStudent(student._id)} style={{ accentColor: 'var(--neon-green)' }} />
                          <span className="text-sm">
                            {student.user?.name || 'Student'} <span style={{ color: 'var(--text-muted)' }}>({student.course || 'N/A'}{student.phone ? `, ${student.phone}` : ''})</span>
                          </span>
                        </label>
                      ))}
                    </div>
              ...
              <div className="flex items-center gap-2 text-xs p-3 rounded-lg"
                style={{ background: 'rgba(0,255,136,0.06)', border: '1px solid rgba(0,255,136,0.15)', color: 'var(--text-secondary)', fontFamily: 'Rajdhani, sans-serif' }}>
                <MessageCircle size={14} /> Sent through direct WhatsApp Cloud API only.
              </div>

              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowWhatsAppModal(false)} className="btn-secondary flex-1">Cancel</button>
                <button type="submit" disabled={whatsAppSending} className="btn-primary flex-1 flex items-center justify-center gap-2">
                  <Send size={14} /> {whatsAppSending ? 'Sending...' : 'Send WhatsApp'}
                </button>
              </div>
            </form>
          </div>
